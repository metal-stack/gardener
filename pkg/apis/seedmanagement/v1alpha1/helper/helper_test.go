	...
	Describe("#ExtractSeedTemplateAndGardenletConfig", func() {
		...
		Context("w/ gardenlet config", func() {
			...
			It("should return the template from `.spec.gardenlet.seedConfig.seedTemplate", func() {
				...
				seedTemplate, gardenletConfig, err := ExtractSeedTemplateAndGardenletConfig(managedSeed.Name, seedmanagementv1alpha1helper.GardenletConfigFromManagedSeed(managedSeed.Spec.Gardenlet))
