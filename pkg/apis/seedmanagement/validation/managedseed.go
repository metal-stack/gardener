...
func validateGardenletConfigUpdate(newConfig, oldConfig runtime.Object, fldPath *field.Path) field.ErrorList {
	...
	if newConfig != nil && oldConfig != nil {
		...
		if err != nil {
			allErrs = append(allErrs, field.Invalid(configPath, newConfig, fmt.Sprintf("could not convert gardenlet config: %v", err)))
			return allErrs
		}
		...
		if err != nil {
			allErrs = append(allErrs, field.Invalid(configPath, oldConfig, fmt.Sprintf("could not convert gardenlet config: %v", err)))
			return allErrs
		}
