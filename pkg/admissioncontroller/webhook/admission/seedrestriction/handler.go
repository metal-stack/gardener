...
func (h *Handler) allowIfGardenletIsNotYetBootstrapped(ctx context.Context, seedName, gardenletNamespace, gardenletName string) admission.Response {
	...
	if err := h.Client.Get(ctx, kubernetesutils.Key(gardenletNamespace, gardenletName), gardenlet); err != nil {
		if apierrors.IsNotFound(err) {
			return admission.Errored(http.StatusForbidden, err)
		}
		return admission.Errored(http.StatusInternalServerError, err)
	...
	seed := &gardencorev1beta1.Seed{}
	if err := h.Client.Get(ctx, kubernetesutils.Key(gardenletName), seed); err != nil {
		if !apierrors.IsNotFound(err) {
			return admission.Errored(http.StatusInternalServerError, err)
		}
		return admission.Allowed("")
