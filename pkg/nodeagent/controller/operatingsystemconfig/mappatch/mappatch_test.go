...
var _ = Describe("mappatch", func() {
	Describe("SetMapEntry", func() {
		It("should set the value into a existing map", func() {
			...
			got, err := mappatch.SetMapEntry(m, mappatch.MapPath{"a", "b", "c"}, func(_ any) (any, error) { return 2, nil })
		...
		It("should populate a nil map", func() {
			...
			got, err := mappatch.SetMapEntry(m, mappatch.MapPath{"a", "b", "c"}, func(_ any) (any, error) { return true, nil })
			Expect(err).NotTo(HaveOccurred())
			Expect(got).To(Equal(want))
		})

		It("should error when there are no path elements", func() {
			got, err := mappatch.SetMapEntry(nil, mappatch.MapPath{}, func(_ any) (any, error) { return true, nil })
		...
		It("should error when the setter function returns an error", func() {
			got, err := mappatch.SetMapEntry(nil, mappatch.MapPath{"a", "b"}, func(_ any) (any, error) { return nil, fmt.Errorf("unable to set value") })
			Expect(err).To(MatchError(fmt.Errorf("unable to set value")))
			Expect(got).To(BeNil())
		})

		It("traversing into a non-map turns into an error", func() {
			got, err := mappatch.SetMapEntry(map[string]any{"a": true}, mappatch.MapPath{"a", "b", "c"}, func(_ any) (any, error) { return nil, nil })
