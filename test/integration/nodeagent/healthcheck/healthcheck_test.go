...
var _ = Describe("Healthcheck controller tests", func() {
	...
	BeforeEach(func() {
		...
		kubeletHealthcheck = healthcheck.NewKubeletHealthChecker(
			mgr.GetClient(), nodeName, clock, fakeDBus, mgr.GetEventRecorderFor(healthcheck.ControllerName), getAddresses,
		)

		containerdHealthcheck := healthcheck.NewContainerdHealthChecker(
			mgr.GetClient(), containerdClient, clock, nodeName, fakeDBus, mgr.GetEventRecorderFor(healthcheck.ControllerName),
		)
