...
// CRICgroupDriverName is a string denoting the cgroup driver for containerd
type CRICgroupDriverName string

const (
	// CRICgroupDriverCgroupfs sets the cgroup driver to cgroupfs
	CRICgroupDriverCgroupfs CRICgroupDriverName = "cgroupfs"

	// CRICgroupDriverSystemd sets the cgroup driver to system
	CRICgroupDriverSystemd CRICgroupDriverName = "systemd"
)
...
type ContainerdConfig struct {
	...
	// Plugins configures configuration beneath the containerd config.toml plugins section.
	// +optional
	Plugins []PluginConfig `json:"plugins,omitempty"`
}

// PluginConfig is a configuration put beneath the containerd config.toml plugins section.
type PluginConfig struct {
	// Path in the configuration that is constructed by the given path elements.
	Path []string `json:"path"`
	// Values are the values configured at the given path.
	// +optional
...
const (
	...
	// ContainerDBaseDir is the path to the configuration base directory of containerd.
	ContainerDBaseDir = "/etc/containerd"
	// ContainerDConfigFile is the path to the config.toml which configures containerd.
	ContainerDConfigFile = ContainerDBaseDir + "/config.toml"
	// ContainerDCertsDir is the certs directory of containerd.
	ContainerDCertsDir = ContainerDBaseDir + "/certs.d"
	// ContainerDConfigDir is the config directory of containerd.
	ContainerDConfigDir = ContainerDBaseDir + "/conf.d"
)
